<?php
	...
	if ( $_POST ) {
		...
		$motivoEstorno = trim( $_POST["motivoEstorno"] );
		$quemEstornou  = trim( $_POST["quemEstornou"] );
		...
		if ( empty( $data ) ) {
			...
			if ( empty ( $id ) ) {
				...
				//dar update
				$sql = "update conta 
					set data = ?, descricao = ?, valor = ?, valorPago = ?, dataPagamento = ?, ativo = 'n', motivoEstorno = ?, quemEstornou = ?, tipo = 'e' where id = ? limit 1";
				$consulta = $pdo->prepare( $sql );
				$consulta->bindParam(1, $data);
				$consulta->bindParam(2, $descricao);
				$consulta->bindParam(3, $valor);
				$consulta->bindParam(4, $valorPago);
				$consulta->bindParam(5, $dataPagamento);
				$consulta->bindParam(6, $motivoEstorno);
				$consulta->bindParam(7, $quemEstornou);
			...
			if ( $consulta->execute() ) {
				...
				echo "<script>alert('Registro Salvo');location.href='listaEstorno';</script>";
